...
class DormantOpponent(PlayerInterface):
    def __init__(self, n_agents):
        ...
        self.p_capture_slow = np.random.uniform(0,.5)
        self.p_return_slow = np.random.uniform(.5,.7)
        self.p_capture_fast = np.random.uniform(.337,1)
        self.p_return_fast = np.random.uniform(.45,1)
        self.p_intercept_slow = np.random.uniform(.5,1)
        self.p_intercept_fast = np.random.uniform(.62,1)
        orb_norm = 0
        ...
        self.p_orbitals = [x/orb_norm for x in orbitals]

        self.p_dodge = np.random.uniform(.8,1)
        self.p_random_traj_change = 0
